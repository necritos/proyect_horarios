...
Child.prototype = {
	...
	getDiv : function() {
		...
		if (this.className == "event") {
			...
			div.style.width = ((this.length * 120) + ((this.length - 1) * 8))
					+ "px";
...
function Timetable(parentDiv) {
	...
	for ( var x = 0; x < 15; x++) {
		var hour = new Child();
		hour.contents = put0b4(x+7) + ":00 &raquo; " + put0b4(x + 8) + ":00";
...
function base60(base100) {
	...
	var mn = Math
			.floor(base100.substring(base100.length - 2, base100.length) / 0.6);
